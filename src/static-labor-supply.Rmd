 ...
 **R1** In the case of heterogenous beta, the regression delivers incorrect estimates because of the omitted variable problem. In this case, beta is no longer a fixed parameter, it's a variable that affects choice of h (and in consequence c), thus we ought to take it into account in the regression. 
...
pander(sfit3)
```
**R2** Adding the participaion decisions doesn't affect the parameters of the labor supply. However, it affects the intercept which becomes larger when we restrict our data to the ones who choose to work. 
...
**R3** Combining two cross sections leads to incorrect estimates as the observations are not consistent with the basic assumptions of panel data analysis. Biases occur in this type of setting. 
...
```
**R5**
We get very biased results, even controlling for Z, again it's not panel. Now we try differences, same as before. 
...
Estimates improve a lot when we difference, we almost recover parameters, same logic. Even though deciles are according to X, we average within each bin and this controls for people's heterogeneity that influences their participation. 
...
We still recover the parameters, but coefficient on dz is not significant anyhow. 
